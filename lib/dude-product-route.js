...
function offer(variant, canonical, policy) {
  ...
  return {
    ...
    ...(policy ? {
      shippingDetails: offerShippingDetails(policy),
...
function brandedProductTitle(value) {
  ...
  return /\bdude mcgee\b/i.test(title) ? title : `Dude McGee ${title}`.trim();
}

function structuredData(product) {
  const canonical = productUrl(product.handle);
  const title = brandedProductTitle(product.title);
  ...
  const productProperties = schemaProductProperties(productAttributes(product, description));
  const common = {
  ...
  if ((product.variants || []).length < 2) {
    ...
    const variantOffer = offer(variant, canonical, policy);
    return { ...common, ...productProperties, ...variantProperties(variant || {}), '@type': 'Product', '@id': `${canonical}#product`, ...(variant && variant.sku ? { sku: variant.sku } : {}), ...(variantOffer ? { offers: variantOffer } : {}) };
  }
  ...
  return {
    ...
    hasVariant: product.variants.map(variant => {
      ...
      return {
        ...
        name: optionValues.length ? `${title} - ${optionValues.join(' / ')}` : title,
        description,
