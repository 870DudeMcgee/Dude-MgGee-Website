...
function renderProduct(product, requestedVariant) {
  ...
  const state = { product, selectedVariantId: selected && variantNumericId(selected) };
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(product.title)} | Dude McGee Merch</title><meta name="description" content="${escapeHtml((description || product.title).slice(0, 160))}"><link rel="canonical" href="${canonical}"><meta property="og:type" content="product"><meta property="og:url" content="${canonical}"><meta property="og:title" content="${escapeHtml(product.title)} | Dude McGee Merch"><meta property="og:description" content="${escapeHtml((description || product.title).slice(0, 200))}">${image ? `<meta property="og:image" content="${escapeHtml(image.url)}">` : ''}<link rel="stylesheet" href="/styles.css"><link rel="stylesheet" href="/merch.css"><style>.product-page-shell{max-width:1180px;margin:0 auto;padding:9rem 2rem 5rem;display:grid;grid-template-columns:minmax(0,1.1fr) minmax(300px,.9fr);gap:3rem}.product-page-image{width:100%;aspect-ratio:1;object-fit:cover;background:#111}.product-page-options fieldset{border:0;padding:0;margin:1.5rem 0}.product-page-options legend{font:600 .75rem 'IBM Plex Mono',monospace;letter-spacing:.08em;text-transform:uppercase}.product-page-options button{margin:.25rem;padding:.6rem .8rem;background:transparent;color:inherit;border:1px solid #777;cursor:pointer}.product-page-options button[aria-pressed=true]{border-color:#e5ff00;color:#e5ff00}.product-page-options button:disabled{opacity:.45;cursor:not-allowed}.product-page-buy{margin-top:1.5rem}@media(max-width:760px){.product-page-shell{grid-template-columns:1fr;padding:7rem 1rem 3rem}}</style><script type="application/ld+json">${json(structuredData(product))}</script></head><body class="merch-page"><a class="skip-link" href="#content">Skip to content</a><header class="site-header"><a class="wordmark" href="/" aria-label="Dude McGee home"><span class="wordmark-dot"></span>Dude McGee</a><nav class="desktop-nav" aria-label="Primary navigation"><a href="/merch.html">Merch</a><a href="/returns.html">Returns</a></nav></header><main id="content" class="product-page-shell"><section><a class="section-index" href="/merch.html">← All merch</a><div class="product-gallery"><img class="product-page-image" id="product-image" src="${escapeHtml(image ? image.url : '')}" alt="${escapeHtml(image ? image.alt : product.title)}"></div></section><section><p class="section-index">${escapeHtml(product.productType || product.vendor || 'Official merchandise')}</p><h1 class="product-title" style="font-size:clamp(2rem,5vw,4.5rem)">${escapeHtml(product.title)}</h1>${description ? `<p class="product-desc">${escapeHtml(description)}</p>` : ''}<p class="product-price" id="product-price">${price}</p><p id="product-availability" aria-live="polite">${selected && selected.available ? 'Available for secure online checkout.' : 'This option is currently sold out.'}</p><div class="product-page-options" id="product-options"></div><button class="product-buy product-page-buy" id="product-buy" type="button"${selected && selected.available ? '' : ' disabled'}>${selected && selected.available ? 'Checkout securely' : 'Sold out'}</button><p class="cart-note">Shipping and taxes are calculated at checkout. <a href="/returns.html">Returns and order support</a></p></section></main><script id="product-data" type="application/json">${json(state)}</script><script src="/assets/product-page.js" defer></script></body></html>`;
}
...
module.exports = { createProductHandler, renderProduct, structuredData, selectedVariant, variantNumericId };
