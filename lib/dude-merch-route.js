...
function renderCatalog(products) {
  ...
  return validProducts.map((product, index) => {
    ...
    const color = /\b(black|white)\b/i.exec(product.title || '');
    const form = catalogForm(product);
    const spec = [color ? color[1].toUpperCase() : '', form].filter(Boolean).join(' / ');
    const price = catalogMoney(product.priceRange && product.priceRange.minVariantPrice);
    const lost = product.availableForSale === false;
    return `<article class="product-card artifact-card" data-server-rendered-card><div class="product-gallery"><div class="product-image-wrap">${image ? `<a class="product-image-zoom" href="${escapeHtml(href)}" aria-label="View ${escapeHtml(product.title)}"><img src="${escapeHtml(image.url)}" alt="${escapeHtml(imageAlt(product, image))}" loading="lazy" decoding="async"></a>` : '<div class="product-no-image" aria-hidden="true">DM</div>'}${lost ? '<span class="product-tag is-soldout">Signal lost</span>' : ''}<span class="product-index">${String(index + 1).padStart(2, '0')} / ${String(validProducts.length).padStart(2, '0')}</span></div></div><div class="product-body">${spec ? `<p class="artifact-spec">${escapeHtml(spec)}</p>` : ''}<h3 class="product-title"><a class="product-title-button" href="${escapeHtml(href)}">${escapeHtml(product.title)}</a></h3>${product.vendor ? `<p class="product-vendor">${escapeHtml(product.vendor)}</p>` : ''}${description ? `<p class="product-desc">${escapeHtml(description)}</p>` : ''}${price ? `<p class="product-price">${escapeHtml(price)}</p>` : ''}<a class="product-buy" href="${escapeHtml(href)}">${lost ? 'Signal lost' : '/// Acquire →'}</a><a class="product-details-trigger" href="${escapeHtml(href)}">View product →</a></div></article>`;
  }).join('');
}
