...
function renderMerchPage(template, products) {
  ...
  return template.replace(gridPattern, replacement);
}
...
function createMerchHandler(getCatalog, readPageTemplate = readTemplate) {
  return async function handler(request, response) {
    ...
    try {
      ...
      const body = renderMerchPage(readPageTemplate(), catalog.products || []);
