...
const { productAttributes, requireProductPolicy } = require('./product-policies');
...
// Business-day attributes are compound country/business_days values. Flat XML
// text was accepted by the feed fetch but flagged on individual Merchant offers.
function shippingXml(policy) {
  const shipping = policy.shipping;
  return `\n      <g:shipping>\n        <g:country>${shipping.country}</g:country>\n        <g:service>${shipping.service}</g:service>\n        <g:price>${shipping.price} ${shipping.currency}</g:price>\n        <g:min_handling_time>${shipping.minHandlingDays}</g:min_handling_time>\n        <g:max_handling_time>${shipping.maxHandlingDays}</g:max_handling_time>\n        <g:min_transit_time>${shipping.minTransitDays}</g:min_transit_time>\n        <g:max_transit_time>${shipping.maxTransitDays}</g:max_transit_time>\n      </g:shipping>\n      <g:shipping_handling_business_days>\n        <g:country>${xml(shipping.country)}</g:country>\n        <g:business_days>${xml(shipping.businessDays)}</g:business_days>\n      </g:shipping_handling_business_days>\n      <g:shipping_transit_business_days>\n        <g:country>${xml(shipping.country)}</g:country>\n        <g:business_days>${xml(shipping.businessDays)}</g:business_days>\n      </g:shipping_transit_business_days>`;
}
...
function additionalImageUrls(product, primaryImage) {
  ...
  return [
    ...
    ...(product.variants || []).map(variant => variant && variant.image),
  ].filter(url => {
    if (!/^https?:\/\//i.test(String(url || '')) || seen.has(url)) return false;
    seen.add(url);
    return true;
  }).slice(0, 10);
}

function itemForVariant(product, variant, schemaVariant, policy) {
  ...
  const fields = {
    ...productAttributes(product, description),
...
function renderGoogleProductFeed(catalog) {
  ...
  for (const product of catalog.products) {
    ...
    const policy = requireProductPolicy(product);
    const schema = structuredData(product);
    const variants = schema.hasVariant || [schema];
    product.variants.forEach((variant, index) => items.push(itemForVariant(product, variant, variants[index], policy)));
  }
