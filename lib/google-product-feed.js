...
const REVIEWED_ADULT_UNISEX_HATS = new Set(['foam-trucker-hat', 'foam-trucker-hat-1']);
...
function catalogFields(product, description) {
  ...
  // Reviewed Printful foam trucker hats are adult, gender-neutral, one-size products.
  if (REVIEWED_ADULT_UNISEX_HATS.has(product.handle)) {
    fields.gender = 'unisex';
    fields.age_group = 'adult';
  }
  return fields;
}
